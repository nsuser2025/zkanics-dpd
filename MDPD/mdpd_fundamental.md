<p>
通常の単一成分系 DPD 流体の状態方程式（EOS）は $\rho$ の二次関数で近似されるため（Ishizuka2023_J.Mol.Liq.vol384.122246.Eq.16）,
$(\partial p/\partial \rho)_{T} > 0$ である.
一般に, 圧力の密度微分が負になる熱力学条件下で気液共存が可能なので, 通常の DPD の EOS ではや自由表面を伴う現象をシミュレーションすることはできない.
